...
CAPTCHA_PATH = NOW_PATH + 'WEBLMT_train/'
LABEL_PATH = NOW_PATH + 'WEBLMT_train/result.txt'
DIVIDE_PATH = NOW_PATH + 'WEBLMT_train_divide/'
TEST_PATH = NOW_PATH + 'WEBLMT_test/'
# TEST_PATH = NOW_PATH + 'WEBLMT_test/'  #测试时is_divide = 0时启用
# TEST_PATH = NOW_PATH + 'divide_test/'
#TEST_PATH = NOW_PATH + 'my_test/temp_test/'
...
class CaptchaTensorFlow(object):
        ...
        for line in fp.readlines():
            ...
            for i in range(CAPTCHA_LENGTH):
                if str.isdigit(str(now_line[i])):
                    col_labels[0, i * CAPTCHA_CLASS +
                               int(str(now_line[i]))] = 1
                else:
                    col_labels[0, i * CAPTCHA_CLASS +
                               ord(str(now_line[i])) - 87] = 1
    ...
    def getDivideFeaturesAndLabels(self):
        ...
        for now_class in self.captcha_class:
            ...
            for x in os.listdir(now_path):
                ...
                now_image = now_image.convert("1")
                self.features[self.sample_num,
                              :] = self.getImagePixel(now_image)
    ...
    def debug(self, is_divide=0):
        ...
        while True:
            ...
            print("\nLabel:", end="")
            if not is_divide:
                print(self.bitsToResult(
                    self.labels[num, :].reshape(1, LABEL_SIZE)))
            else:
                print(self.bitsToResultDivide(
                    self.labels[num, :].reshape(1, DIVIDE_LABEL_SIZE)))
            input()
            num += 1
    ...
    def initCNNConv(self, pre_conv, channels_in, channels_out, index):
        with tf.name_scope('conv' + str(index)):
            ...
            W_conv = self.weightVariable(
                [3, 3, channels_in, channels_out], name='W_conv' + str(index))
            W_max = tf.argmax(W_conv, )
            # 计算偏置
            b_conv = self.biasVariable(
                [channels_out], name='b_conv' + str(index))
            # 卷积层
            h_conv = tf.nn.relu(tf.nn.bias_add(self.conv2d(
                pre_conv, W_conv), b_conv), name='h_conv' + str(index))
            # 池化层
    ...
    def initCNN(self, is_divide=0):
        ...
        self.X = tf.placeholder(
            tf.float32, [None, feature_size], name='features')
        self.Y = tf.placeholder(tf.float32, [None, label_size], name='labels')
        self.keep_prob = tf.placeholder(tf.float32, name='keep_prob')
        # 转化成4d
        x_image = tf.reshape(
            self.X, [-1, feature_height, feature_weight, 1], name='x_image')
        ...
        with tf.name_scope('flat'):
            # feature_size / 64 * 256
            W_fc1 = self.weightVariable(
                [feature_size // 64 * 256, 1080], name='W_fc1')
            b_fc1 = self.biasVariable([1080], name='b_fc1')
            h_pool3_flat = tf.reshape(
                h_pool3, [-1, feature_size // 64 * 256], name='h_pool3_flat')
            h_fc1 = tf.nn.relu(
                tf.add(tf.matmul(h_pool3_flat, W_fc1), b_fc1), name='h_fc1')
            h_fc1_drop = tf.nn.dropout(
                h_fc1, self.keep_prob, name='h_fc1_drop')
        ...
        with tf.name_scope('output'):
            ...
            y_conv = tf.nn.softmax(
                tf.matmul(h_fc1_drop, W_fc2) + b_fc2, name='y_conv')
    ...
    def train(self, is_divide=0):
        ...
        with tf.name_scope('loss'):
            # 加1e-10是必要的
            loss = -tf.reduce_sum(self.Y * tf.log(y_conv + 1e-10))
        tf.summary.scalar('loss', loss)

        # optimizer 为了加快训练
        optimizer = tf.train.AdamOptimizer(
            learning_rate=self.learn_rate).minimize(loss)

        if not is_divide:
            predict = tf.reshape(y_conv, [-1, CAPTCHA_LENGTH, CAPTCHA_CLASS])
            max_idx_p = tf.argmax(predict, 2)
            max_idx_l = tf.argmax(tf.reshape(
                self.Y, [-1, CAPTCHA_LENGTH, CAPTCHA_CLASS]), 2)
            ...
            with tf.name_scope('accuracy'):
                correct_prediction = tf.equal(
                    tf.argmax(y_conv, 1), tf.argmax(self.Y, 1))
        ...
        while step < 2001:
            ...
            now_summary, now_loss, _ = SESS.run([merged_summary, loss, optimizer], feed_dict={
                                                self.X: batch_x, self.Y: batch_y, self.keep_prob: 0.9})
            ...
            if step % 50 == 0:
                ...
                now_summary, acc = SESS.run([merged_summary, accuracy], feed_dict={
    ...
    def getPredictResult(self, y):
        ...
        for i in range(CAPTCHA_LENGTH):
            now_list = y[0, i *
                         CAPTCHA_CLASS: (i + 1) * CAPTCHA_CLASS].tolist()
    ...
    def getTestFeature(self, is_divide=0):
        if not is_divide:
            ...
            self.test_num = (len(os.listdir(TEST_PATH)) -
                             1) // 2 * CAPTCHA_LENGTH
            self.test_feature = np.zeros((self.test_num, DIVIDE_IMAGE_SIZE))
        ...
        for i in range(num):
            ...
            if not is_divide:
                ...
                for j in range(CAPTCHA_LENGTH):
                    divide_image = now_image.crop(
                        (j * DIVIDE_IMAGE_WEIGHT, 0, (j + 1) * DIVIDE_IMAGE_WEIGHT, DIVIDE_IMAGE_HEIGHT))
                    self.test_feature[i * CAPTCHA_LENGTH + j,
                                      :] = self.getImagePixel(divide_image)
    ...
    def loadModel(self):
        ...
        self.test_keep_prob = graph.get_operation_by_name(
            'keep_prob').outputs[0]
    ...
    def Test(self, is_divide=0):
        ...
        for i in range(loop_num):
            if not is_divide:
                res = SESS.run(self.test_y, feed_dict={self.test_x: self.test_feature[i, :].reshape(
                    1, feature_size), self.test_keep_prob: 1.})
                pred_res = self.getPredictResult(res[0].reshape(1, label_size))
                true_res = self.bitsToResult(
                    self.test_labels[i, :].reshape(1, label_size))
            else:
                pred_res, true_res = "", ""
                begin_time = time.time()
                for j in range(CAPTCHA_LENGTH):
                    res = SESS.run(self.test_y, feed_dict={
                                   self.test_x: self.test_feature[i * CAPTCHA_LENGTH + j, :].reshape(1, feature_size), self.test_keep_prob: 1.})
                    pred_res += self.getDividePredictResult(
                        res[0].reshape(1, label_size))
                    true_res += self.bitsToResultDivide(
                        self.test_labels[i * CAPTCHA_LENGTH + j, :].reshape(1, label_size))
                end_time = time.time()
    ...
    def mytest(self):
        self.loadModel()
        TEST_PATH = NOW_PATH + 'my_test/temp_test2'
        labels = ["c","m","q","v","4", "7", "9", "e","k"]
        # labels = ["c","4"]
        suffix = ".png"
        feature_size = DIVIDE_IMAGE_SIZE
        label_size = DIVIDE_LABEL_SIZE
        for label in labels:
            test_filename = os.path.join(TEST_PATH, label+suffix)
            origin_img = Image.open(test_filename)
            origin_img = origin_img.convert("1")
            # origin_img.save(test_filename)
            # continue
            # origin_img.show()
            # plt.figure("origin")
            ...
            #test_img = origin_img.resize((DIVIDE_IMAGE_WEIGHT, DIVIDE_IMAGE_HEIGHT))
            # test_img.show()
            # plt.figure("resize")
            # plt.imshow(origin_img)
            # plt.show()
            test_feature = np.zeros(DIVIDE_IMAGE_SIZE)
            test_feature[0:DIVIDE_IMAGE_SIZE] = self.getImagePixel(test_img)

            res = SESS.run(self.test_y, feed_dict={self.test_x: test_feature.reshape(
                1, feature_size), self.test_keep_prob: 1.})
            pred_res = self.getDividePredictResult(
                res[0].reshape(1, label_size))
